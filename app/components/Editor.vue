...
function copyToClipboard() {
    ...
    toast.add({
        ...
        description: `${t("toasts.copiedDesc", { lang: isOriginal ? sourceLanguageName.value : "English" })}`,
        color: "info",
    });
}
...
async function downloadZip() {
    ...
    const zip = new JSZip();
    zip.file(
        `${sourceLanguageName.value.toLowerCase()}.json`,
        JSON.stringify(results.value.original, null, 2),
    );
...
<template>
    <div class="min-h-screen bg-transparent font-sans py-10">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            <div class="flex flex-col gap-3">
                <div class="flex justify-between items-center px-1 min-h-6">
                    <div class="flex gap-4 items-center">
                        ...
                        <div class="flex gap-2 items-center">
                            <span
                                v-if="inputText"
                                class="text-[10px] text-slate-400 italic animate-pulse"
                            >
                                Draft saved
                            </span>
                            <span
                ...
                <UAlert
                    ...
                    variant="subtle"
                    title="Tip"
                    :description="$t('editor.tip')"
                    class="mb-1"
                />

                <div
                    class="h-125 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-white dark:bg-[#131926] shadow-sm focus-within:ring-2 focus-within:ring-primary-500/50 transition-all"
                >
                    <UTextarea
                        v-model="inputText"
                        :rows="24"
                ...
                <div class="flex justify-between items-center px-1">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <UButton
                            v-if="
                                inputText.trim() && isJsonInput && isValidJson
                            "
            ...
            <div class="flex flex-col gap-3">
                ...
                <div
                    class="relative h-125 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#131926] overflow-hidden shadow-sm flex flex-col"
                >
                    <Transition
                    ...
                    >
                        <div
                            v-if="isLoading"
                            key="loading"
                            class="absolute inset-0 p-6 flex flex-col gap-4 bg-white/80 dark:bg-[#0B0F1A]/80 z-20 backdrop-blur-[2px]"
                        >
                        ...
                        >
                            <div
                                class="flex p-1 bg-slate-100 dark:bg-slate-800/50 rounded-lg shrink-0"
                            >
                                <button
                            ...
                            <div
                                class="flex-1 overflow-hidden rounded-lg bg-[#0B0F1A] border border-slate-800 shadow-inner"
                            >
                                <div
                                    class="h-full overflow-y-auto p-4 font-mono text-[11px] leading-relaxed"
                                >
                                    <pre
                                        v-if="activeTab === 'original'"
                                        class="text-emerald-400 whitespace-pre-wrap"
                                        >{{
                                            JSON.stringify(
                                                results.original,
                                                null,
                                    ...
                                    <pre
                                        v-if="activeTab === 'en'"
                                        class="text-sky-400 whitespace-pre-wrap"
                                        >{{
                            ...
                            <div
                                class="shrink-0 pt-2 flex justify-between items-center border-t border-slate-100 dark:border-slate-800"
                            >
                                <UButton
                ...
                <p class="px-1 text-[10px] text-slate-400 italic text-right">
                    {{ $t("result.footerNote") }}
                </p>
            </div>
        </div>
        ...
        <div class="my-16 text-center">
            <p
                class="text-slate-500 dark:text-slate-400 text-lg max-w-2xl mx-auto leading-relaxed"
            >
                {{ $t("guide.title") }}
            </p>
        </div>
